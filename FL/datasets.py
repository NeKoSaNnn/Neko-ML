...
class InitDataSet(object):
    ...
    def get(self):
        ...
        if self.args.dataset == "mnist":
            self.train_dataset = datasets.MNIST(root=self.dataset_path, train=True, transform=self.trans, download=True)
            self.test_dataset = datasets.MNIST(root=self.dataset_path, train=False, transform=self.trans, download=True)
        elif self.args.dataset == "cifar10":
            self.train_dataset = datasets.CIFAR10(root=self.dataset_path, train=True, transform=self.trans,
                                                  download=True)
            self.test_dataset = datasets.CIFAR10(root=self.dataset_path, train=False, transform=self.trans,
                                                 download=True)
        else:
