...
def predict(config_path, weights_path, predict_img_path, ground_truth_path, out_threshold=0.5):
    ...
    img = Image.open(predict_img_path).resize((img_size, img_size))
    _, tensor_img = utils.to_tensor_use_pil(predict_img_path, img_size)
    tensor_img = tensor_img.unsqueeze(0)
    tensor_img = tensor_img.to(device)

    ground_truth = Image.open(ground_truth_path).resize((img_size, img_size))
    ...
    utils.draw_predict(num_classes, img, ground_truth, predict_mask, save_path)
